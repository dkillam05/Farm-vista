...
(function () {
  ...
  if (window.__FV_ELEVATOR_GRADE_FIX_20260916_5) return;
  window.__FV_ELEVATOR_GRADE_FIX_20260916_5 = true;
  ...
  if (!window.__FV_GRAIN_TICKET_FULL_RES_CAPTURE_20260916) {
    ...
    HTMLCanvasElement.prototype.toBlob = function(callback, type, quality) {
      ...
      if (mime === 'image/jpeg' || mime === 'image/jpg') return originalToBlob.call(this, callback, type, 1.0);
  ...
  const scoularTemplateReady = (() => {
    ...
    return new Promise(resolve => {
      ...
      if (existing) {
        if (window.FVGrainTicketTemplates?.scoularWaverly) return resolve(true);
        existing.addEventListener('load', () => resolve(true), { once:true });
        existing.addEventListener('error', () => resolve(false), { once:true });
        return;
      }
      const script = document.createElement('script');
      script.src = '/js/grain-ticket-templates/scoular-waverly.js?v=20260916-3';
  ...
  function documentText(data,root) { return clean(root?.documentText || data?.documentText || root?.document?.text || data?.document?.text || data?.result?.document?.text || data?.ocrResult?.document?.text || root?.grainTicket?.rawText || ''); }
  ...
  function clearResolvedScoularErrors(root, result) {
    if (!Array.isArray(root.scanErrors)) root.scanErrors = [];
    root.scanErrors = root.scanErrors.filter(message => {
      const s = String(message || '').toLowerCase();
      if (result.grades && (s.includes('test weight') || s.includes('moisture') || s.includes('damage') || s.includes('foreign material') || s.includes('grade'))) return false;
      if (result.weights && (s.includes('gross minus tare') || s.includes('gross weight') || s.includes('tare weight') || s.includes('net weight'))) return false;
      if (result.bushels && s.includes('bushel')) return false;
      return true;
    });
  ...
  function patchScoular(data) {
    ...
    const text=documentText(data,root); const template=window.FVGrainTicketTemplates?.scoularWaverly; if(!template)return false;
    ...
    clearResolvedScoularErrors(root,result);
    if (!result.complete) {
      root.scanValid = false;
      if (!root.scanErrors.length) root.scanErrors.push('Scoular Waverly ticket could not be fully verified from OCR. Please review.');
    }
    console.log('[Grain Ticket] Scoular Waverly GitHub template result:',result);
    return result.changed || result.matched;
  }
