...
// Rev: 2026-09-11-weather-zip-modal-only
//
// Dashboard weather card -> modal wiring.
// ZIP editing is intentionally available ONLY inside the weather details modal.
// A ZIP entered there is persisted by fv-weather.js and immediately becomes the
// location used by both the details modal and the dashboard weather tile.
...
(function () {
  ...
  async function renderMainWeather(location) {
    ...
    if (!apiKey) {
      console.warn("Weather: dashboard API key is not available yet.");
      return;
    }
