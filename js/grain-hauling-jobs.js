...
function ticketIsVoided(ticket){return ticket?.voided===true||norm(ticket?.status).includes("void")}
function contractIsVoided(contract){return contract?.voided===true||norm(contract?.status||contract?.contractStatus).includes("void")}
function installVoidGuardStyles(){if(document.getElementById("fv-hauling-void-guard-style"))return;const style=document.createElement("style");style.id="fv-hauling-void-guard-style";style.textContent=`#void-hauling-job-btn:disabled,#void-hauling-job-btn.fv-void-disabled{background:rgba(179,38,30,.12)!important;color:rgba(157,36,30,.58)!important;border:1px solid rgba(179,38,30,.22)!important;box-shadow:none!important;cursor:not-allowed!important;opacity:1!important}`;document.head.appendChild(style)}
function comboSelectFromButton(button){return button?.closest?.(".fv-combo")?.querySelector?.("select")||null}
function removeMisplacedSoldUnderRows(){const customerId="hauling-job-customer";Array.from(document.querySelectorAll(".fv-panel.show")).forEach(panel=>{const ownerId=clean(panel.dataset?.fvSelectId||panel.dataset?.selectId||panel.getAttribute?.("data-for"));if(ownerId===customerId)return;panel.querySelectorAll(".fv-item").forEach(item=>{if(clean(item.textContent)==="+ Add New Sold Under")item.remove()})})}
function installSoldUnderComboScopeGuard(){let activeHaulingSelectId="";document.addEventListener("click",event=>{const button=event.target.closest?.(".fv-buttonish"),select=comboSelectFromButton(button);if(!select?.id?.startsWith("hauling-job-"))return;activeHaulingSelectId=select.id;if(activeHaulingSelectId!=="hauling-job-customer"){queueMicrotask(removeMisplacedSoldUnderRows);setTimeout(removeMisplacedSoldUnderRows,0);setTimeout(removeMisplacedSoldUnderRows,50)}},true);new MutationObserver(()=>{if(activeHaulingSelectId&&activeHaulingSelectId!=="hauling-job-customer")removeMisplacedSoldUnderRows()}).observe(document.body,{childList:true,subtree:true})}
...
installVoidGuardStyles();installSoldUnderComboScopeGuard();installContractJobPropagation();document.addEventListener("click",e=>{if(e.target.closest?.("#edit-hauling-job-btn,[data-edit-hauling-job]"))setTimeout(syncVoidButton,150)},true);
