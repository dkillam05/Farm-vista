...
async function loadLatestReadiness(state, { force=false } = {}){
  ...
  try{
    ...
    const last =
      Number(state._latestReadinessLoadedAt || 0);
...
function buildSyntheticRunFromLatest(
  state,
  fieldObj,
  latestRec
){
  ...
  return {
    ...
    drainageIndex:
      safeNum(rec.drainageIndex),

    readinessCreditIn:
      safeNum(rec.readinessCreditIn) ?? 0,
    ...
    surfacePenaltyFinal:
      safeNum(rec.surfacePenaltyFinal),

    wetBiasApplied:
      safeNum(rec.wetBiasApplied),
    ...
    weatherSource:
      safeStr(rec.weatherSource),

    timezone:
      safeStr(rec.timezone),

    computedAtISO:
      safeStr(rec.computedAtISO),

    weatherFetchedAtISO:
      safeStr(rec.weatherFetchedAtISO),
