...
export async function loadFieldDocs(force=false){
  ...
  const fresh = !force && appState.fieldsCache.data.length && (Date.now() - appState.fieldsCache.loadedAt) < CACHE_TTL_MS;
...
export async function loadFarmDocs(force=false){
  ...
  const fresh = !force && appState.farmsCache.data.length && (Date.now() - appState.farmsCache.loadedAt) < CACHE_TTL_MS;
...
export async function loadMrmsDocs(force=false){
  ...
  const fresh = !force && appState.mrmsCache.data.length && (Date.now() - appState.mrmsCache.loadedAt) < CACHE_TTL_MS;
...
export async function loadPersistedStateMap(force=false){
  const now = Date.now();
  if (!force && appState.readinessState._persistLoadedAt && (now - appState.readinessState._persistLoadedAt) < 30000){
