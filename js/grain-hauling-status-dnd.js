/* FarmVista — Grain Contracts hauling-job status DND
   ...
   Replaces the failed Show All Jobs button with a Status dropdown inside the
   existing Ticket -> Hauling Job filter row. Matching Jobs remains the clean
   default. Choosing a status opens an alternate right-side DND list without
   fighting the core contextual renderer.

   Manual ticket moves here change ONLY hauling-job assignment. Contract / Spot
   allocations are intentionally preserved.
*/
(() => {
  'use strict';

  if (window.__FV_HAULING_STATUS_DND_20260912) return;
  window.__FV_HAULING_STATUS_DND_20260912 = true;
  ...
  function installStyle() {
    if (document.getElementById('fv-hauling-status-dnd-style')) return;
    const style = document.createElement('style');
    style.id = 'fv-hauling-status-dnd-style';
    style.textContent = `
      ...
      #contracts-table-body td.fv-remain-green{
        background:rgba(59,126,70,.16)!important;color:#24602f!important;font-weight:900!important
      }
      #hauling-jobs-table-body td.fv-remain-orange,
      #contracts-table-body td.fv-remain-orange{
        background:rgba(230,126,34,.19)!important;color:#9b4c00!important;font-weight:900!important
      }
      #hauling-jobs-table-body td.fv-remain-red,
      #contracts-table-body td.fv-remain-red{
        background:rgba(179,38,30,.16)!important;color:#981f1a!important;font-weight:900!important
      }
  ...
  function ticketsForJob(jobId) {
    return Array.from(state.tickets.values())
      ...
      .sort((a,b) => clean(b?.ticketDate || b?.date).localeCompare(clean(a?.ticketDate || a?.date)));
  }
  ...
  function jobStatus(job) {
    ...
    const ticketed = ticketsForJob(job.id).reduce((sum,t) => sum + ticketBushels(t),0);
    ...
    return `
      <div class="fv-hauling-ticket-card" data-fv-status-ticket="1" data-ticket-id="${esc(ticket.id)}" draggable="true">
        ...
        <div>
          ...
          <div class="fv-hauling-ticket-meta">${esc(clean(ticket?.ticketDate || ticket?.date || ticket?.deliveryDate) || 'No date')} • ${esc(clean(ticket?.crop || ticket?.commodity || ticket?.grain || ticket?.cropName) || 'Unknown crop')}<br>${esc(ticketBuyer(ticket) || ticketLocation(ticket) || 'Unknown destination')}${ticketLocation(ticket) && ticketBuyer(ticket) ? ` • ${esc(ticketLocation(ticket))}` : ''}</div>
          <div class="fv-ticket-sold-under"><strong>Sold Under:</strong> ${esc(soldUnder(ticket))}</div>
          <div class="fv-status-ticket-note">Drag left to unassign from this hauling job, or onto another compatible hauling job. Contract / Spot allocation is not changed.</div>
        </div>
      </div>`;
  }
  ...
  function jobMarkup(job) {
    ...
    const target = jobTarget(job);
    const ticketed = tickets.reduce((sum,t) => sum + ticketBushels(t),0);
    const remaining = Math.max(0,target-ticketed);
    const over = Math.max(0,ticketed-target);
    const status = jobStatus(job);
    return `
      <div class="fv-ticket-job-card fv-status-job-card ${status}${over > .005 ? ' overhauled' : ''}" data-fv-status-job="1" data-fv-ticket-job-id="${esc(job.id)}">
        <div class="fv-ticket-job-title">${esc(jobName(job))}<span class="fv-status-pill ${status}">${esc(statusLabel(status))}</span></div>
        <div class="fv-ticket-job-meta">${esc(jobBuyer(job) || 'Unknown buyer')} • ${esc(jobLocation(job) || 'Unknown location')} • ${esc(jobCrop(job) || 'Unknown crop')}<br>${fmtBu(ticketed)} bu ticketed • ${fmtBu(remaining)} bu remaining${over > .005 ? ` • <span class="fv-status-over">${fmtBu(over)} bu over</span>` : ''} • ${tickets.length} ticket${tickets.length === 1 ? '' : 's'}</div>
        <div class="fv-job-sold-under"><strong>Sold Under:</strong> ${esc(soldUnder(job))}</div>
        ${tickets.length ? `<div class="fv-ticket-job-linked"><div class="fv-ticket-job-linked-label">Assigned Tickets</div>${tickets.map(ticketMarkup).join('')}</div>` : '<div class="fv-ticket-job-empty">No tickets assigned yet.</div>'}
      </div>`;
  }

  function ensureStatusUI() {
    const old = document.getElementById('fv-show-all-hauling-jobs');
    if (old) old.remove();
  ...
  function renderOverride() {
    ...
    if (message) {
      message.textContent = `${state.mode === 'all' ? 'Showing all hauling jobs' : `Showing ${statusLabel(state.mode)} hauling jobs`}. Drag assigned tickets between jobs or back to Unassigned.`;
  ...
  async function assignTickets(ids,jobId) {
  ...
  function bindOverrideDnd() {
    ...
    alt.querySelectorAll('[data-fv-status-job][data-fv-ticket-job-id]').forEach(card => {
      card.addEventListener('dragover',event => { if (!state.dragIds.length) return; event.preventDefault(); card.classList.add('drag-over'); });
      card.addEventListener('dragleave',() => card.classList.remove('drag-over'));
      card.addEventListener('drop',event => {
        event.preventDefault();
        card.classList.remove('drag-over');
        const raw = clean(event.dataTransfer?.getData('text/plain'));
        const ids = raw ? raw.split(',').map(clean).filter(Boolean) : [...state.dragIds];
        state.dragIds = [];
        assignTickets(ids,card.dataset.fvTicketJobId);
      });
    });

    if (left.dataset.fvStatusUnassign !== '1') {
      left.dataset.fvStatusUnassign = '1';
  ...
  function touchTarget(x,y) {
    ...
    return under?.closest?.('[data-fv-status-job][data-fv-ticket-job-id],#fv-unassigned-ticket-list') || null;
  }
  ...
  document.addEventListener('pointermove',event => {
    if (!state.touch.active) {
      if (state.touch.timer) {
        if (Math.abs(event.clientX-state.touch.startX) > 12 || Math.abs(event.clientY-state.touch.startY) > 12) clearTouch();
      }
      return;
    }
  ...
  document.addEventListener('pointerup',event => {
    ...
    clearTouch();
    if (jobId) assignTickets(ids,jobId); else if (unassign) unassignTickets(ids);
  ...
  document.addEventListener('change',event => {
    if (['fv-ticket-filter-buyer','fv-ticket-filter-sold-under','fv-ticket-filter-crop'].includes(event.target?.id) && state.mode !== 'matching') queue();
  },true);
